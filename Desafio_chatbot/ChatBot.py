...
# Define o locale para português (pode variar conforme o sistema)
try:
    ...
    if city:
        try:
            ...
            if 'error' in data:
                ...
                # Conversão para datetime e formatação
                dt = datetime.strptime(last_updated, '%Y-%m-%d %H:%M')
                weekday = dt.strftime('%A')  # Dia da semana
                time_str = dt.strftime('%H:%M')  # Horário
                ...
                with col2:
                    ...
                    st.write(f"**Atualizado em:** {weekday}, {time_str}")
